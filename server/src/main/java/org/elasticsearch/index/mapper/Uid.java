...
public final class Uid {
    ...
    static boolean isURLBase64WithoutPadding(String id) {
        ...
        switch (length & 0x03) {
            ...
            case 1:
                return false;
            case 2:
                // 最后2个符号（12位）编码1个字节（8位）
                // 所以最后一个符号实际上只使用8-6 = 2位，只能取4个值
                ...
                break;
            case 3:
                // 最后3个符号（18位）编码2个字节（16位）
                // 所以最后一个符号实际上只使用16-12 = 4位，只能取16个值
    ...
    /** 使用数字id，我们只需在单个字节中折叠两个连续的字符，并使用0x0f作为结束标记。 */
